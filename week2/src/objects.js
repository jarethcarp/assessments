  ...
  let bugArr = []
    for (let bug of bugs){
      ...
      bugMonth.forEach((month) => {
        ...
        let currentBug = [];
        currentBug.push(bug.name)
        console.log("currentBug", currentBug)
        console.log("bugArr", bugArr)
        console.log("bugCalendar[month]", bugCalendar[month])
        if (bugCalendar[month]){
          console.log("bugCalendar[month] is True")
          for (let arrLen = 0; arrLen<=bugArr.length-1; arrLen++){
            currentBug.push(bugArr[arrLen])
            console.log("currentBug", bugArr)
          }
          bugCalendar[month] = currentBug
          bugArr = currentBug
          console.log("BugCalendar ", bugCalendar)
        } else{
          console.log("bugCalendar[month] is False")
          bugCalendar[month] = currentBug
          bugArr = currentBug
        }
      })
